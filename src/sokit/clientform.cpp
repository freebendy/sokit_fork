...
ClientForm::ClientForm(QWidget* p, Qt::WFlags f)
    :BaseForm(p, f),m_client(0),mTimerId(0)
{
    m_ui.setupUi(this);
    QIntValidator* frequencyValidator = new QIntValidator(this);
    frequencyValidator->setBottom(1);
    m_ui.repeat_frequency_lineEdit->setValidator(frequencyValidator);
    connect(m_ui.repeat_btn,
            SIGNAL(toggled(bool)),
            this,
            SLOT(onToggled(bool)));
}
...
void ClientForm::onToggled(bool checked)
{
//    qDebug() << "ClientForm::onToggled - " << checked;
    if (checked)
    {
        int frequency = m_ui.repeat_frequency_lineEdit->text().toInt();
        mTimerId = startTimer(frequency);
    }
    else
    {
        if (mTimerId != 0)
...
void ClientForm::timerEvent(QTimerEvent* aTimerEvent)
{
//    qDebug() << "ClientForm::timerEvent - "<< aTimerEvent->timerId() ;
    if (aTimerEvent->timerId() == mTimerId)
    {
        QString data = m_ui.buf_lineEdit->text();
        if (m_client && lock(1000))
        {
            m_client->send(data);
            unlock();
